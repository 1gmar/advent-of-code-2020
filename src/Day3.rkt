...
(define input-parser
  (let* ([return-vector (λ (lst) (return (apply vector-immutable lst)))]
         [line-parser (>>= (many1 (choice (list (char #\.) (char #\#)))) return-vector)])
...
(define (count-trees-for-slope map-matrix slope)
  (let* ([row-step (first slope)]
         ...
         [is-tree? (λ (pos) (let ([row (first pos)] [column (modulo (second pos) width)])
                                 (equal? #\# (vector-ref (vector-ref map-matrix row) column))))])
        (length (filter is-tree? (takef (for/list ([i (in-range height)])
